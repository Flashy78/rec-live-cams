...
class MyFreeCams(Plugin):
    ...
    _data_schema = validate.Schema(
        {
            ...
            "uid": int,
            "vs": int,
            validate.optional("u"): {"camserv": int},
        }
    )
    ...
    def _php_fallback(self, username, user_id, php_message):
        ...
        _data_php_re = re.compile(
            ...
            + r"""(?P<uid>{0}),(?P<vs>\d+),[^,]+,[^,]+,(?P<camserv>\d+)[^\]]+\]""".format(
                _uid_php_re
            )
        )
        ...
        data = {
            ...
            "u": {"camserv": int(match.group("camserv"))},
        }
        return data
    ...
    def _get_streams(self):
        ...
        for s in HLSStream.parse_variant_playlist(self.session, HLS_VIDEO_URL).items():
            yield s

        if DASH_VIDEO_URL and self.get_option("dash"):
            log.debug("DASH URL: {0}".format(DASH_VIDEO_URL))
            for s in DASHStream.parse_manifest(self.session, DASH_VIDEO_URL).items():
                yield s
