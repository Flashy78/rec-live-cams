...
unpack_packer_re = re.compile(r"""(?P<data>eval\(function\(p,a,c,k,e,(?:d|r)\).*\))""")
unpack_unescape_re = re.compile(
    r"""
    <script[^<>]*>[^>]*
...
unpack_source_url_re_1 = re.compile(
    r"""(?x)source:\s*(?P<replace>window\.atob\(
    (?P<q>["'])(?P<atob>[A-z0-9+/=]+)(?P=q)\)),\s*
    mimeType:\s*["']application/vnd\.apple\.mpegurl["']
"""
)
unpack_source_url_re_2 = re.compile(
    r"""(?x)var\s\w+url=(?P<replace>atob\(
    (?P<q>["'])(?P<atob>[A-z0-9+/=]+)(?P=q)\));"""
)
unpack_source_url_re_3 = re.compile(
    r"""(?x)Clappr\.Player\({\s*
...
class Generic(Plugin):
    ...
    def compare_url_path(self, parsed_url, check_list, path_status="startswith"):
        status = False
        for netloc, path in check_list:
            if path_status == "==":
                ...
                if parsed_url.netloc.endswith(netloc) and parsed_url.path.startswith(
                    path
                ):
                    status = True
                    break
        ...
        for url in old_list:
            ...
            if new_url in GenericCache.cache_url_list and not allow_same_url:
                ...
                and parse_new_url.netloc.endswith(
                    tuple(self.get_option("whitelist_netloc"))
                )
                is False
            ):
                ...
                REMOVE = "BL-static"
            elif self.get_option("blacklist_netloc") and parse_new_url.netloc.endswith(
                ...
                self.compare_url_path(parse_new_url, GenericCache.blacklist_path)
                is True
            ):
                ...
                REMOVE = "BL-ew"
            elif self.get_option("blacklist_filepath") and parse_new_url.path.endswith(
                tuple(self.get_option("blacklist_filepath"))
            ):
                # Removes blacklisted file paths
                # --generic-blacklist-filepath
                REMOVE = "BL-filepath"
            elif self._ads_path_re.search(
                parse_new_url.path
            ) or parse_new_url.netloc.startswith(("ads.")):
                # Removes obviously AD URL
                REMOVE = "ADS"
            elif (
                self.compare_url_path(
                    parse_new_url, blacklist_path_same, path_status="=="
                )
                is True
            ):
                ...
                parse_new_url.netloc == "cdn.embedly.com"
                and parse_new_url.path == "/widgets/media.html"
            ):
    ...
    def _resolve_playlist(self, playlist_all):
        ...
        for url in playlist_all:
            ...
            if parsed_url.path.endswith((".m3u8")) or parsed_url.query.endswith(
                (".m3u8")
            ):
            ...
            elif parsed_url.path.endswith((".mpd")) or parsed_url.query.endswith(
                (".mpd")
            ):
    ...
    def _res_text(self, url):
        try:
            ...
            if "Received response with content-encoding: gzip" in str(e):
                ...
                log.error(
                    "Website Access Denied/Forbidden, you might be geo-"
    ...
    def get_title(self):
        if self.title is None:
            ...
            _og_title_re = re.compile(
                r'<meta\s*property="og:title"\s*content="(?P<title>[^<>]+)"\s*/?>'
            )
                ...
                streams.append(
                    (
                        ...
                        MuxedStream(
                            self.session,
                            HTTPStream(
                                self.session, video_url, headers=stream["http_headers"]
                            ),
                            HTTPStream(
                                self.session, audio_url, headers=stream["http_headers"]
                            ),
                        ),
                    )
                )
    ...
    def _get_streams(self):
        if (
            HAS_YTDL
            and not self.get_option("ytdl-disable")
            and self.get_option("ytdl-only")
        ):
        ...
        if self._run <= 1:
            log.info(
                "Version {0} - https://github.com/back-to/generic".format(
                    GENERIC_VERSION
                )
            )
        ...
        if self.get_option("debug"):
            _valid_filepath = re.sub(
                r"(?u)[^-\w.]", "", str(self.url).strip().replace(" ", "_")
            )
            _new_file = os.path.join(
                Path().absolute(), f"{self._run}_{_valid_filepath}.html"
            )
        ...
        if iframe_list:
            ...
            new_iframe_list = self._make_url_list(
                iframe_list, self.url, url_type="iframe"
            )
            if new_iframe_list:
                ...
                if number_iframes == 1:
                    ...
                    try:
                        number = int(
                            self.input_ask("Choose an iframe number from above").split(
                                " "
                            )[0]
                        )
        ...
        if (
            HAS_YTDL
            and not self.get_option("ytdl-disable")
            and not self.get_option("ytdl-only")
        ):
            ___streams = self.ytdl_fallback()
            if ___streams and len(___streams) >= 1:
                return (s for s in ___streams)
