...
schema = validate.Schema({"status": "success"})


@pluginmatcher(re.compile(r"(http(s)?://)?(\w{2}.)?(bongacams\d*?\.com)/([\w\d_-]+)"))
class bongacams(Plugin):
    def _get_streams(self):
        ...
        listing_url = urlunparse(
            (stream_page_scheme, stream_page_domain, LISTING_PATH, "", "", "")
        )
        ...
        if response.status_code != 200:
            ...
            raise PluginError(
                "unexpected status code for {0}: {1}".format(
        ...
        if not model_name.lower() in list(
            [model["username"].lower() for model in response["models"]]
        ):
        ...
        if not esid:
            raise PluginError(
                "unknown error, esid={0} for {1}.\nResponse: {2}".format(
                    esid, model_name, response["models"]
                )
            )
